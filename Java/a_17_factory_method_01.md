...
- 팩토리 메서드의 많은 장점들이 있겠지만, 내가 프로그래밍을 하면서 직접적으로 느낀점을 정리해보려고 한다.
  1. **유연한 인스턴스 생성** : 생성 과정이 복잡한 인스턴스를 생성하기 용이하다.
     - 인스턴스를 생성할 때 필요한 과정과 자원들을 이용해서 인스턴스를 생성해주는 로직을 팩토리에 맡길 수 있다.
     - 가령, 중복되는 과정은 다른 팩토리로 빼거나 여러가지 자원을 조합해서 인스턴스를 생성하는 것 등등 팩토리의 역할을 심화시킬 수 있다.
  ...
  3. **SRP (단일책임의 원칙)** : 객체를 생성하는 책임을 하나의 클래스에 부여함으로써 가독성, 유지보수성이 좋아진다.
